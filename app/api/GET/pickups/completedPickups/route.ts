...
export async function GET(request: NextRequest) {
  ...
  const res = await fetch(`${process.env.URLB}/orders?status=completed`, {
    cache: 'no-cache',
    headers: {
  ...
  if (!res.ok) {
    return Response.json({ success: false, error: 'Failed to fetch completed pickups' }, { status: res.status });
  }
