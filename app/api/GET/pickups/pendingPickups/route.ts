...
export async function GET(request: NextRequest) {
  ...
  const res = await fetch(`${process.env.URLB}/orders?status=pending`, {
    cache: 'no-cache',
    headers: {
  ...
  if (!res.ok) {
    return Response.json({ success: false, error: 'Failed to fetch pending pickups' }, { status: res.status });
  }
