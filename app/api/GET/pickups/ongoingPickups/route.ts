...
export async function GET(request: NextRequest) {
  ...
  const res = await fetch(`${process.env.URLB}/orders?status=ongoing`, {
    cache: 'no-cache',
    headers: {
  ...
  if (!res.ok) {
    return Response.json({ success: false, error: 'Failed to fetch ongoing pickups' }, { status: res.status });
  }
